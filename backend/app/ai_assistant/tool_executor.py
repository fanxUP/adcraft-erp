...
class ToolExecutor:
    ...
    async def execute_confirmed_action(self, action_id, user, ip_address=None, user_agent=None):
        pending = await self.action_confirm.confirm_action(action_id)
        if not pending:
        ...
        if not tool_def or not tool_def.handler:
            await self.action_confirm.mark_executed(action_id, error_message=f"未知工具: {pending.tool_name}")
        ...
        try:
            ...
            await self.audit_logger.log_audit(
                ...
                business_type=pending.tool_args.get("business_type"),
                business_id=pending.tool_args.get("business_id"),
