...
class Product(Base, TimestampMixin):
    ...
    default_loss_rate: Mapped[float] = mapped_column(Numeric(8, 4), default=0, comment="默认损耗率")
...
class Material(Base, TimestampMixin):
    ...
    # CDR 扩展字段
    thickness_mm: Mapped[float | None] = mapped_column(Numeric(8, 2), nullable=True, comment="厚度(mm)")
    sheet_width_mm: Mapped[float | None] = mapped_column(Numeric(10, 2), nullable=True, comment="单张/单卷宽度(mm)")
    sheet_height_mm: Mapped[float | None] = mapped_column(Numeric(10, 2), nullable=True, comment="单张/单卷高度(mm)")
    purchase_unit: Mapped[str | None] = mapped_column(String(32), nullable=True, comment="采购单位")
    min_purchase_qty: Mapped[float] = mapped_column(Numeric(14, 3), default=0, comment="最小采购量")
...
class Process(Base, TimestampMixin):
    ...
    startup_fee: Mapped[float] = mapped_column(Numeric(14, 2), default=0, comment="开机费")
    min_charge: Mapped[float] = mapped_column(Numeric(14, 2), default=0, comment="最低收费")
    standard_hours: Mapped[float | None] = mapped_column(Numeric(8, 2), nullable=True, comment="标准人工工时")
    remark: Mapped[str | None] = mapped_column(Text, nullable=True)
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
