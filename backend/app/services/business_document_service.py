...
class BusinessDocumentService:
    ...
    async def reopen_completed_order(self, doc_id: UUID, reason: str, operated_by: UUID) -> dict:
        ...
        if not open_task:
            ...
            self.db.add(task)
            # The task collection was eagerly loaded before this method. Keep
            # the in-memory response consistent with the task created above;
            # otherwise the UI would reopen successfully but still show only
            # the old completed installation tasks until a manual refresh.
            doc.installation_tasks.append(task)
            await self.db.flush()

        # BusinessDocument.updated_at is generated by the database on flush.
        # Refresh it before _to_detail() so async SQLAlchemy does not attempt
        # an implicit scalar reload while serializing the response.
        await self.db.refresh(doc, attribute_names=["updated_at"])
        return self._to_detail(doc)
    ...
    async def set_cost(self, doc_id: UUID, cost_amount: float) -> dict:
        ...
        # BusinessDocument.updated_at is generated by the database on flush.
        # Refresh it before serializing so async SQLAlchemy does not attempt an
        # implicit scalar reload outside a greenlet (MissingGreenlet -> HTTP 500).
        await self.db.refresh(doc, attribute_names=["updated_at"])
        return self._to_detail(doc)
