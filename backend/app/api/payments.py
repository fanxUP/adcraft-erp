...
async def download_project_cost_template(
    ...
    try:
        ...
        headers = ["分项", "日期", "供应商", "支付金额", "欠款金额", "分类", "付款方式", "支出总额", "说明"]
        col_widths = [20, 18, 24, 14, 14, 18, 16, 14, 30]
        ...
        example_data = ["灯箱制作", "2026-07-08", "XX材料供应商", 500.00, 0.00, "材料费", "转账支付", 500.00, "示例数据，可删除"]
        ...
        note_cell = ws.cell(row=4, column=1,
            value="说明：支出总额 = 支付金额 + 欠款金额；分类可选值 — 人工/工时费、材料费、租赁费、运输/物流费、安装杂费、办公费、餐费/交通费、差旅费、其他；付款方式可选值 — 现金支付、微信支付、支付宝转账、转账支付、对公支付、其它支付；供应商必须先在供应商管理中建立。凭证请在成本记录保存后上传。")
