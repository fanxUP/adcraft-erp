<template>
  <AppPage>
    <template #header><PageHeader title="备份管理" description="创建、导入、导出和恢复数据库备份。恢复操作会覆盖当前数据库，请谨慎执行。">
      <template #actions><el-button @click="fetchList" :loading="loading">刷新</el-button></template>
    </PageHeader></template>
    ...
    <!-- Actions -->
    <PageToolbar aria-label="备份操作">
      <template #actions>
      <el-button @click="handleCreate" :loading="creating" :disabled="creating" type="primary">
    ...
    <!-- Table -->
    <DataTableShell :state="tableState" aria-label="备份文件列表">
      ...
      <template #error><StatePanel state="error" action-label="重新加载" @action="fetchList" /></template>
    </DataTableShell>
...
const loadError = ref('')
const tableState = computed(() => loading.value ? 'loading' : loadError.value ? 'error' : backups.value.length ? 'ready' : 'empty')
...
async function fetchList() {
  ...
  try {
    ...
    loadError.value = getErrorMessage(error, '获取备份列表失败')
