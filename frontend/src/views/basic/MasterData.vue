      ...
      <el-tab-pane label="产品管理" name="products">
        <div class="tab-content">
          ...
          <el-table :data="prod.list" v-loading="prod.loading" stripe style="margin-top: 16px">
            ...
            <el-table-column label="默认单价" width="100"><template #default="{ row }">¥ {{ row.default_price?.toFixed(2) }}</template></el-table-column>
            ...
            <el-table-column label="操作" width="160">
              <template #default="{ row }">
                <el-button text type="primary" @click="prod.openEdit(row)">编辑</el-button>
      ...
      <el-tab-pane label="材质管理" name="materials">
        <div class="tab-content">
          ...
          <el-table :data="mat.list" v-loading="mat.loading" stripe style="margin-top: 16px">
            ...
            <el-table-column label="操作" width="160">
              <template #default="{ row }">
                <el-button text type="primary" @click="mat.openEdit(row)">编辑</el-button>
      ...
      <el-tab-pane label="工艺管理" name="processes">
        <div class="tab-content">
          ...
          <el-table :data="proc.list" v-loading="proc.loading" stripe style="margin-top: 16px">
            ...
            <el-table-column label="操作" width="160">
              <template #default="{ row }">
                <el-button text type="primary" @click="proc.openEdit(row)">编辑</el-button>
      ...
      <el-tab-pane label="供应商管理" name="suppliers">
        <div class="tab-content">
          ...
          <el-table :data="sup.list" v-loading="sup.loading" stripe style="margin-top: 16px">
            ...
            <el-table-column label="供应类型" width="100"><template #default="{ row }"><el-tag size="small">{{ supplyTypeLabel(row.supply_type) }}</el-tag></template></el-table-column>
            <el-table-column prop="is_active" label="状态" width="70">
              <template #default="{ row }"><el-tag :type="row.is_active ? 'success' : 'info'" size="small">{{ row.is_active ? '启用' : '禁用' }}</el-tag></template>
            </el-table-column>
            <el-table-column label="操作" width="180">
              <template #default="{ row }">
                <el-button text type="primary" @click="sup.openEdit(row)">编辑</el-button>
      ...
      <el-form :model="prod.form" ref="prod.formRef" :rules="prod.rules" label-width="100px">
        ...
        <el-form-item label="规格尺寸">
          <div style="display: flex; gap: 8px; width: 100%; flex-wrap: wrap">
            ...
            <span style="line-height: 32px; margin: 0 4px">面积</span>
            <el-input-number v-model="prod.form.area" :precision="3" :min="0" style="width: 100px" />
            <span style="line-height: 32px; margin: 0 4px">数量</span>
...
function onTabChange(tab: string) {
  activeTab.value = tab
  if (tab === 'materials') mat.fetchData()
  else if (tab === 'processes') proc.fetchData()
  else if (tab === 'suppliers') sup.fetchData()
  else if (tab === 'products') prod.fetchData()
}
...
function useProductTab() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editingId.value) { await updateProduct(editingId.value, payload); ElMessage.success('更新成功') }
  ...
  async function handleDelete(row: ProductResponse) {
    try {
      ...
      await deleteProduct(row.id); ElMessage.success('已删除'); fetchData()
    } catch { /* ignore */ }
  }

  return { loading, saving, list, total, page, pageSize, keyword, dialogVisible, editingId, formRef, form, rules, materialOptions, processOptions, loadOptions, fetchData, handleCreate, openEdit, handleSave, handleDelete }
}
...
function useMaterialTab() {
  ...
  function openEdit(row: MaterialResponse) {
    editingId.value = row.id
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editingId.value) { await updateMaterial(editingId.value, payload); ElMessage.success('更新成功') }
      else { await createMaterial(payload); ElMessage.success('创建成功') }
  ...
  async function handleDelete(row: MaterialResponse) {
    try {
      ...
      await deleteMaterial(row.id); ElMessage.success('已删除'); fetchData()
    } catch { /* ignore */ }
  }

  return { loading, saving, list, total, page, pageSize, keyword, dialogVisible, editingId, form, fetchData, handleCreate, openEdit, handleSave, handleDelete }
}
...
function useProcessTab() {
  ...
  function openEdit(row: ProcessResponse) {
    editingId.value = row.id
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editingId.value) { await updateProcess(editingId.value, payload); ElMessage.success('更新成功') }
      else { await createProcess(payload); ElMessage.success('创建成功') }
      dialogVisible.value = false; fetchData()
    } finally { saving.value = false }
  }
  ...
  async function handleDelete(row: ProcessResponse) {
    try {
      ...
      await deleteProcess(row.id); ElMessage.success('已删除'); fetchData()
    } catch { /* ignore */ }
  }
...
function useSupplierTab() {
  ...
  async function handleDelete(row: SupplierResponse) {
    try {
      ...
      await deleteSupplier(row.id); ElMessage.success('已删除'); fetchData()
    } catch { /* ignore */ }
  }

  return { loading, saving, list, total, page, pageSize, keyword, dialogVisible, editingId, formRef, form, rules, fetchData, handleCreate, openEdit, handleSave, handleDelete }
}
